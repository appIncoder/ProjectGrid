...
export class ProjectTechnologyManagement implements OnChanges, OnDestroy {
  ...
  private buildLanes(): ParentActivityLaneVm[] {
    ...
    const matrix = ((project as any).activityMatrix ?? (project as any).taskMatrix) as
      ...
      | undefined;
    const parentRow = matrix?.['technologie'] ?? {};
    const childMatrix = ((project as any).projectTasksMatrix?.['technologie'] ?? {}) as
      | Record<string, Record<string, Task[]>>
      | undefined;
  ...
  async saveTaskModal(): Promise<void> {
    ...
    await this.mutateAndPersist(async () => {
      if (this.isCreateMode) {
        ...
        if (!matrix.technologie[phase][nextParentId]) matrix.technologie[phase][nextParentId] = [];
        const taskId = this.generateTaskId('technologie', phase, nextParentId, nextName);
        const created: Task = {
          id: taskId,
          label: nextName,
          status: 'todo',
  ...
  private ensureProjectTasksMatrix(): void {
    ...
    if (!p.projectTasksMatrix.technologie || typeof p.projectTasksMatrix.technologie !== 'object') {
      p.projectTasksMatrix.technologie = {};
    }

    const activityMatrix = (p.activityMatrix ?? p.taskMatrix ?? {}) as Record<string, Record<string, Task[]>>;
    const parentRow = activityMatrix?.['technologie'] ?? {};
    const phases = Array.isArray(p.phases) ? p.phases : [];

    for (const phase of phases) {
      if (!p.projectTasksMatrix.technologie[phase] || typeof p.projectTasksMatrix.technologie[phase] !== 'object') {
        p.projectTasksMatrix.technologie[phase] = {};
      }
      const parents = Array.isArray(parentRow[phase]) ? parentRow[phase] : [];
      for (const parent of parents) {
        const parentId = String(parent?.id ?? '').trim();
        if (!parentId) continue;
        if (!Array.isArray(p.projectTasksMatrix.technologie[phase][parentId])) {
          p.projectTasksMatrix.technologie[phase][parentId] = [{ ...parent, parentActivityId: parentId, comments: [] }];
        } else {
          for (const t of p.projectTasksMatrix.technologie[phase][parentId]) {
  ...
  private getProjectTasksMatrix(): Record<'technologie', Record<PhaseId, Record<string, Task[]>>> {
    return ((this.project as any).projectTasksMatrix ?? {}) as Record<'technologie', Record<PhaseId, Record<string, Task[]>>>;
  }

  private findTaskRef(phase: PhaseId, parentId: string, taskId: string): { list: Task[]; index: number; task: Task } | null {
    this.ensureProjectTasksMatrix();
    const matrix = this.getProjectTasksMatrix();
    const list = matrix?.technologie?.[phase]?.[parentId];
  ...
  private findParentActivityIdForTask(phase: PhaseId, taskId: string): string | null {
    const matrix = this.getProjectTasksMatrix();
    const phaseMap = matrix?.technologie?.[phase] ?? {};
  ...
  private getParentActivitiesForPhase(phase: PhaseId): Array<{ id: string; label: string }> {
    ...
    const parents = Array.isArray(matrix?.['technologie']?.[phase]) ? matrix['technologie'][phase] : [];
    return parents
